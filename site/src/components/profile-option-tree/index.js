...
const OptionField = ({ option, onMenuClick, onOptionChange }) => {
  ...
  const { fields: { checked, name, price } } = option;
  const form = option.menuState === 'choose'
    ...
    <>
      <div className='checkbox'>
        ...
        <label for={option.id}>{name.value}</label>
      </div>
      <p>{price.value}/ea</p>
    </>
    : null;
...
const ProfileOptionTree = ({ options, onMenuClick, onOptionChange }) => {
  ...
  const copyAndClimb = useCallback((payload, { option, options, activeOption, activeOptions }, handler) => {
    ...
    // call the recursive handler set in the inner ProfileOptionTree onMenuClick below
    handler(payload, {
      option,
      options: tier,
      activeOption,
    ...
    <>
      {options.map(option =>
        <Option
          ...
          onOptionChange={(name, checked) => handleOptionChange({ name, checked }, { option, options })}
        >
...
export default ProfileOptionTree;